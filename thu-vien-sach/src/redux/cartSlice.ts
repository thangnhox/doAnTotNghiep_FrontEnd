...
export const cartSlice = createSlice({
    ...
    reducers: {
        AddBookToCart: (state, action) => {
            if(state.data.books.some(val => val.BookID === action.payload.BookID)) return;
        ...
        RemoveBookFromCart: ( state, action ) => {
            const newBooksArray =  state.data.books.filter((item) => item.BookID !== action.payload.BookID);
            state.data = {
                ...state.data,
                books: newBooksArray,
                total: getTotalPrice(newBooksArray)
...
export const {AddBookToCart, AddDiscountIdToCart, RemoveBookFromCart, RemoveDiscountIdToCart} = cartSlice.actions
